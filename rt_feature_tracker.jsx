...
const PHASES = [
  {
    ...
    features: [
      ...
      { id: "f46", name: "Fix saveConfig duplicate goal code check (F15 bug)", status: "done", priority: "P0", category: "Admin", description: "Fixed (commit 1e7902c): duplicate check now keys on clientId|code instead of code alone, so the same code (e.g. F15) can be assigned to different clients as separate goal entries without tripping the error." },
      // v4 (Sept 2026)
      { id: "f47", name: "Client sheet auto-provisioning + Verify", status: "done", priority: "P1", category: "Admin", description: "Admin console creates + links an app-owned Google Sheet per client (provisionClientSheet) — no manual sharing needed. Verify Sheet button reports reachability + tabs. Editor helpers backfillMissingTrialSummaries / checkTrialSummaryHealth." },
      { id: "f48", name: "Pause & resume sessions (cross-device)", status: "done", priority: "P1", category: "Clinical", description: "Therapist pauses for any interruption (travel, break, bathroom, connectivity) and resumes on ANY device. Paused time EXCLUDED from billable duration (active-time accounting); never-paused sessions unchanged. State stored in Suspended Sessions tab (lossless JSON) + local backup; resume cards on the client screen." },
      { id: "f49", name: "Offline capture + auto-submit queue + live backup", status: "done", priority: "P0", category: "Infrastructure", description: "Delivers f25. Continuous 15s local auto-save (crash recovery); offline Submit queues and auto-sends on reconnect (online event + login); live cross-device backup of in-progress sessions (stale >3min surfaced for recovery). Logout warns on unsynced pause / queued submit." },
      { id: "f50", name: "Idempotent submissions (submissionId dedup)", status: "done", priority: "P0", category: "Data", description: "Stable per-session submissionId (preserved across pause/resume/re-auth/devices). processSession dedups via _sessionAlreadyRecorded (scans Time In Time Out) — no duplicate rows on retry or cross-device completion." },
      { id: "f51", name: "Semantic layer / data dictionary (BigQuery)", status: "done", priority: "P1", category: "Data", description: "analyst/semantic: schema.json machine-readable dictionary (entities, columns, join keys, metrics, PHI flags) + apply_descriptions.sql (BQ table/column descriptions) + de-identified report views. Makes the warehouse model-understandable for LHBM + NL analytics." },
      { id: "f52", name: "Read-only BigQuery MCP + analyst skill", status: "done", priority: "P1", category: "Data", description: "Scaffolded .mcp.json (read-only) + bq-analyst skill for conversational analytics. OAuth connector path documented for BCBA (claude.ai custom connector) + Anthropic BAA guidance. Delivers part of f26 (analyst API surface)." },
      { id: "f53", name: "Looker: de-identified views + dashboard spec", status: "done", priority: "P1", category: "Data", description: "7 report-ready de-identified views (billing, therapist hours, goal progress, behavior trend, authorization utilization, mastery) + 6-page dashboard build spec + Linking-API pre-seed URL." },
      // Pending
      { id: "f25", name: "Offline mode (works without WiFi, sync on reconnect)", status: "done", priority: "P0", category: "Infrastructure", description: "Delivered by f49: data collection works offline (in-memory + 15s localStorage auto-save); offline Submit queues and auto-sends on reconnect; live cross-device backup. (Uses localStorage rather than IndexedDB.)" },
